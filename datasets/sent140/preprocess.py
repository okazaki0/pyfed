...
class sentiment140(Dataset):
    ...
    def line_to_indices(self, line, max_words=25):
        ...
        indl = [
            self.indd[w] if w in self.indd else unk_id for w in line_list[:max_words]
        ]
        length = len(indl)
...
def load_sent140(args):
    ...
    if args.split_mode == "niid":
        dataset = sentiment140(
            DATA_ROOT_TRAIN, word_emb_arr, indd, vocab, sum(args.data_size)
        )
    else:
        dataset = sentiment140(
            DATA_ROOT_TRAIN, word_emb_arr, indd, vocab, data_size=100000
        )
