...
async def fit(args,traced_model,worker_instances,device,test_loader,logger,es):
    ...
    for curr_round in range(1, args.training_rounds + 1):
        ...
        worker_instance = random.sample(worker_instances, k=int(args.clients*args.fraction_client))
        try :
        ...
        test_models = curr_round % args.eval_every == 1 or curr_round == args.training_rounds
        if test_models:
            logger.info("Evaluating models")
            np.set_printoptions(formatter={"float": "{: .0f}".format})
            for worker_id, worker_model, _ in results:
                res = test("Worker " + worker_id ,worker_model,args.loss,device,test_loader,logger)
        ...
        for worker_id, worker_model, worker_loss in results:
            if worker_model is not None:
                ...
                logger.info("Worker {} , training loss : {:.4f}".format(worker_id,loss_values[worker_id]))
        #aggregation
        ...
        if test_models:
            res = test("Federated model",traced_model,args.loss,device,test_loader,logger)
