...
def defineArgs():
    ...
    # number of clients
    parser.add_argument(
        "--clients", "-c", type=int, default=5, help="number of clients (default=5)"
    )
    # client port
    parser.add_argument(
        "--client_port",
        ...
        type=int,
        default=8000,
        help="the client port (default=8000)",
    )
    ...
    parser.add_argument(
        ...
        type=bool,
        default=False,
        help="the server hold also a part of data or not (default=False)",
    )
    ...
    parser.add_argument(
        ...
        type=str,
        default="iid",
        help="split mode: 'iid' or 'niid' (default=iid)",
    )
    ...
    parser.add_argument(
        ...
        type=bool,
        default=False,
        help="--iid_share=true to share samples between clients in the iid split mode (default=False)",
    )
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.1,
        help="the percentage of samples to share between clients in the iid split mode (default=0.1)",
    )
    ...
    parser.add_argument(
        ...
        type=str,
        default="random",
        help="in the non iid split there are 2 type : 'random' and 'label' for split by label (default=random)",
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=[1, 4, 2, 4, 3],
        help="the number of labels for each client (default=[1,4,2,4,3] ",
    )
    ...
    parser.add_argument(
        ...
        type=bool,
        default=False,
        help="add a percentage for the global dataset for all clients (default=False)",
    )
    ...
    parser.add_argument(
        ...
        type=bool,
        default=False,
        help="add some error on data for each client withe a rate",
    )
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.1,
        help="the percentage of data which we want to add (default 0.1)",
    )
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.01,
        help="the percentage of error which we want to add (default 0.01)",
    )
    # import args form config.yaml file
    parser.add_argument(
        "--config_file", "-f", type=str, help="the path of your config file"
    )
    ...
    if args.split_mode == "niid":
        if args.type == "label":
            if args.server_data == True and args.clients != len(args.label_num) - 1:
                print(
                    "Error: so such server_data is True he length of label_num has to equal tothe clients number +1, and the last case is for the server"
                )

            elif args.server_data == False and args.clients != len(args.label_num):
                print(
                    "Error: the number of label_num has to equal to the clients number"
                )
                return
            ...
            if args.server_data == True:
                if len(args.data_size) != args.clients + 1:
                    print(
                        "Error: so such server_data is True he length of dataset_size has to equal to the clients number +1, and the last case is for the server"
                    )
                    return
            else:
                if len(args.data_size) != args.clients:
                    print(
                        "Error: the number of data_size has to equal to the clients number"
                    )
                    return
...
def trainArgs():
    ...
    parser.add_argument(
        "--batch_size", "-bs", type=int, default=32, help="batch size of the training"
    )
    parser.add_argument(
        "--max_nr_batches",
        "-mbs",
        type=int,
        default=10,
        help="max nbr of batch of the training",
    )
    parser.add_argument(
        "--test_batch_size",
        "-ts",
    ...
    parser.add_argument(
        "--save_model", "-smdl", action="store_true", help="if set, model will be saved"
    )
    parser.add_argument(
        "--verbose",
        "-v",
    ...
    parser.add_argument(
        "--dataset", "-d", default="mnist", help="dataset holded by users"
    )
    parser.add_argument(
        "--eval_every",
        "-ee",
        type=int,
        default=10,
        help="evaluate the model evrey n rounds",
    )
    parser.add_argument(
        "--fraction_client",
        "-fc",
        type=float,
        default=1,
        help="Number of clients that will in each round",
    )
    parser.add_argument(
        "--model", "-m", default="cnn_mnist", help="the model that we will use."
    )
    parser.add_argument(
        "--loss",
        "-l",
        default="nll_loss",
        help="the loss function  nll_loss or cross_entropy",
    )
    parser.add_argument(
        "--optimizer",
        "-o",
        default="SGD",
        help="the optimizer that we will use : SGD or Adam",
    )
    parser.add_argument(
        "--aggregation",
        "-a",
        type=str,
        default="federated_avg",
        help="type of aggregation : federated_avg or wieghted_avg",
    )
    parser.add_argument(
        "--data_size",
        "-ds",
    ...
    parser.add_argument(
        "--config_file", "-f", type=str, help="the path of the config file"
    )
    parser.add_argument(
        "--client_port",
        "-cp",
